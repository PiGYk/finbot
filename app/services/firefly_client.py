...
class FireflyClient:
    ...
    async def list_categories(self, limit: int = 500) -> List[dict]:
        data = await self.request("GET", "/api/v1/categories", params={"limit": limit})
        return data.get("data", [])

    async def list_category_names(self) -> List[str]:
        items = await self.list_categories(limit=500)
        names: List[str] = []
        for item in items:
            attrs = item.get("attributes", {})
            name = attrs.get("name")
    ...
    async def ensure_category(self, name: str) -> bool:
    ...
    async def ensure_categories(self, names: List[str]) -> Dict[str, Any]:
        existing = set(await self.list_category_names())
        created: List[str] = []
