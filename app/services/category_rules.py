...
class CategoryRulesService:
        ...
        with self._lock:
            ...
            for rule in rules:
                if self._normalize(rule.get("canonical_name", "")) == normalized_canonical:
                    ...
                    break
            if existing is None:
                existing = {"canonical_name": canonical_name, "aliases": []}
    ...
    def _should_learn_alias(self, alias: str, canonical_name: str) -> bool:
        text = str(alias or "").strip()
        if not text:
            return False
        norm = self._normalize(text)
        if not norm or norm == self._normalize(canonical_name):
            return False
        blocked = {
            "інше",
            "без категорії",
    ...
    def learn_from_correction(
        ...
        aliases: List[str] = []
        for candidate in [old_category, old_description, old_destination]:
            if self._should_learn_alias(str(candidate or ""), new_category):
                aliases.append(str(candidate).strip())
        if not aliases:
            return None
        return self.upsert_rule(new_category, aliases)
