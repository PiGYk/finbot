...
ВАЖЛИВО для зміни категорії:
- Якщо користувач каже "зміни категорію X на Y" або "зміни категорію з X на Y":
  - target_category = X (стара категорія, яку потрібно знайти)
  - category = Y (нова категорія)
- Якщо користувач каже просто "зміни категорію на Y" (без вказівки старої):
  - target_category = null
  - category = Y (нова категорія)

Приклади:
- "зміни категорію їжа на фастфуд" → {{"action": "update", "target_category": "їжа", "category": "фастфуд"}}
- "зміни категорію останньої транзакції їжа на фастфуд" → {{"action": "update", "target_category": "їжа", "category": "фастфуд"}}
- "зміни категорію на продукти" → {{"action": "update", "category": "продукти"}}
- "змінити суму на 500" → {{"action": "update", "amount": 500}}
- "видали останню" → {{"action": "delete", "count": 1}}
...
- Якщо зовсім незрозуміло — використай "{user_text}" як є"""
    ...
    async def categorize_receipt_items(
        ...
        # Беремо список категорій з category_rules якщо є, інакше — дефолтний
        if self.category_rules:
            available = self.category_rules.list_canonical_categories()
        else:
            available = [
                "Продукти", "Овочі та фрукти", "М'ясо та риба", "Їжа", "Кава",
                "Алкоголь", "Цигарки", "Солодкі напої", "Напої", "Вода",
                "Фастфуд і снеки", "Кафе та ресторани",
                "Гігієна та догляд", "Косметика", "Побутова хімія", "Товари для дому",
                "Аптека", "Медичні послуги", "Тварини", "Одяг", "Техніка",
                "Пальне", "Машина", "Транспорт", "Розваги", "Навчання",
                "Підписки та цифрові сервіси", "Сервіси", "Хобі",
                "Канцтовари", "Догляд за собою", "Відеоігри",
                "Житло та комунальні", "Подарунки", "Бізнес / вкладення", "Інше",
            ]
        categories_str = ", ".join(available)
...
Доступні категорії (використовуй ТІЛЬКИ їх, точна назва):
{categories_str}

Правила:
- Обирай НАЙТОЧНІШУ категорію з наведеного списку
- НЕ вигадуй нові категорії — тільки з наведеного списку
...
[
  {{"name": "назва з входу", "category": "точна назва з списку", "confidence": 0.95}},
  ...
]"""
